...
ROOT=Path(__file__).resolve().parents[1];CORE=Path(__file__).with_name("build_site_core.py");DEFAULT_OUTPUT=ROOT/"_site"
...
def assert_core_safeguards()->None:
    source=CORE.read_text(encoding="utf-8");required=("write_generated_sitemap(output)",'git", "log", "-1", "--format=%cs"');missing=[marker for marker in required if marker not in source]
...
def publish_registered_composition(output:Path)->None:
    for relative_path in registered_paths():
        ...
        if relative_path=="index.html" and 'class="platform"' not in composed:
            ...
            composed=compose_product_front_door(composed);composed=apply_home_command_bridge(composed);composed=restore_compact_site_guide(composed)
...
def publish_sitewide_navigation(output:Path)->None:
    ...
    for target in sorted(output.rglob("*.html")):
        ...
        if relative in {"404.html","about/index.html","advisory/index.html","contact/index.html","ethics-charter/index.html","evidence/index.html","experience/index.html","faq/index.html","governance/index.html"}:continue
