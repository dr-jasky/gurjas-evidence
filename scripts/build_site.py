...
def resolved_output(argv: list[str]) -> Path:
    parser=argparse.ArgumentParser(add_help=False);parser.add_argument("--output",type=Path,default=DEFAULT_OUTPUT);args,_=parser.parse_known_args(argv)
...
def assert_core_safeguards() -> None:
    ...
    if missing: raise RuntimeError("Delegated builder is missing indexing safeguards: "+", ".join(missing))
...
def publish_registered_composition(output: Path) -> None:
    for relative_path in registered_paths():
        ...
        if relative_path=="index.html":
            # The new Research Intelligence homepage is an intentional product surface.
            # Keep the shared build/evidence composition, but do not overwrite it with the legacy front-door generator.
            if 'class="platform"' not in composed:
                composed,removed=COMPACT_GUIDE_RE.subn("",composed,count=1)
                if removed!=1:raise RuntimeError("Homepage shared route helper is missing before composition")
...
def git_value(*args: str) -> str:return subprocess.check_output(["git",*args],cwd=ROOT,text=True).strip()
...
def publish_release_manifest(output: Path) -> None:
    ...
    source_date=git_value("show","-s","--format=%cI",source_commit);source_ref=os.environ.get("GITHUB_REF_NAME","").strip() or git_value("rev-parse","--abbrev-ref","HEAD")
    site_data=json.loads((ROOT/"site"/"data"/"site.json").read_text(encoding="utf-8"))
    manifest={"schemaVersion":1,"site":"https://gurjas.org/","sourceCommit":source_commit,"sourceDate":source_date,"sourceRef":source_ref,"navigationVersion":site_data.get("navigationVersion"),"buildSystem":"scripts/build_site.py"}
