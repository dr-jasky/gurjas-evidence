...
from tool_standard_composition import compose_tool_standard, tool_standard_paths
...
def publish_registered_composition(output: Path) -> None:
    ...
    for relative_path in registered_paths():
        ...
        composed = compose_tool_standard(relative_path, composed)
        if 'data-tool-standard="1"' in composed:
            standard_count += 1
    ...
    if standard_count != 11:
        raise RuntimeError(f"Expected eleven governed tool-standard panels, composed {standard_count}")
    print("Published registered composition, including eleven tool-standard panels, the homepage product front door and tool evidence loop")
