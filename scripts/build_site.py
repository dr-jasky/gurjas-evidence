...
TOOLS_OPERATING_SYSTEM_STYLESHEET = '<link rel="stylesheet" href="../assets/tools-operating-system.css?v=110">'
...
def registered_paths() -> tuple[str, ...]:
    return tuple(sorted(set(composed_paths()) | set(tool_evidence_paths()) | set(tool_standard_paths())))


def assert_core_safeguards() -> None:
    source = CORE.read_text(encoding="utf-8")
    required = ('write_generated_sitemap(output)', 'git", "log", "-1", "--format=%cs"')
...
def normalize_platform_landmarks(document: str) -> str:
    """Make labelled platform containers valid ARIA regions.

    The homepage and Research Desk are composed in multiple passes. Some
    composition modules can add their labelled containers after an earlier
    normalization pass, so this function deliberately matches the class even
    when attribute order or additional classes differ.
    """
    pattern = re.compile(
        r'<div(?=[^>]*\bclass="[^"]*\b(?:home-proof-ticker|home-tool-groups|rd-suggestions)\b[^"]*")'
        r'(?=[^>]*\baria-label=)(?![^>]*\brole=)([^>]*)>',
        re.IGNORECASE,
    )

    def add_region(match: re.Match[str]) -> str:
        return '<div role="region"' + match.group(1) + '>'

    return pattern.sub(add_region, document)


def assert_no_invalid_labelled_divs(output: Path) -> None:
    """Fail before deployment if the known platform containers regress."""
    pattern = re.compile(
        r'<div(?=[^>]*\bclass="[^"]*\b(?:home-proof-ticker|home-tool-groups|rd-suggestions)\b[^"]*")'
        r'(?=[^>]*\baria-label=)(?![^>]*\brole=)[^>]*>',
        re.IGNORECASE,
    ...
    for target in output.rglob("*.html"):
        ...
        if pattern.search(document):
            failures.append(target.relative_to(output).as_posix())
    if failures:
        raise RuntimeError("Invalid labelled platform divs remain: " + ", ".join(failures))
...
def normalize_generated_platform_pages(output: Path) -> None:
    for target in output.rglob("*.html"):
        ...
        document = target.read_text(encoding="utf-8")
        document = normalize_platform_landmarks(document)
        if relative == "research-desk/index.html":
            document = document.replace('<main id="main">', '<main>', 1)
        target.write_text(document, encoding="utf-8")
    assert_no_invalid_labelled_divs(output)
...
def publish_registered_composition(output: Path) -> None:
    for relative_path in registered_paths():
        ...
        composed = normalize_platform_landmarks(composed)
        if relative_path == "research-desk/index.html":
            composed = composed.replace('<main id="main">', '<main>', 1)
    ...
    print("Published registered composition, including the homepage product front door, shared tool standard and Research Library evidence loop")
...
def publish_sitewide_navigation(output: Path) -> None:
    ...
    for target in sorted(output.rglob("*.html")):
        relative = target.relative_to(output).as_posix()
        if relative in {"404.html", "about/index.html", "advisory/index.html", "contact/index.html", "ethics-charter/index.html", "evidence/index.html", "experience/index.html", "faq/index.html", "governance/index.html"}:
            continue
        source = target.read_text(encoding="utf-8")
        ...
        if not match:
            raise RuntimeError(f"{relative}: governed primary navigation is missing")
        items = match.group("items")
        if "has-sub" in items or "subnav" in items or "Our Work" in items:
            raise RuntimeError(f"{relative}: obsolete dropdown navigation remains")
        prefix = "../" * max(0, len(Path(relative).parts) - 1)
        normalized = "".join(f'<li><a href="{prefix}{path}">{label}</a></li>' for path, label in EXPECTED_NAVIGATION)
        start, end = match.span("items")
        source = source[:start] + normalized + source[end:]
        if source.count("data-site-guide") != 1:
            raise RuntimeError(f"{relative}: expected exactly one compact route helper")
        if 'assets/site-nav-compact.css?v=1' not in source:
            raise RuntimeError(f"{relative}: compact navigation stylesheet is missing")
...
def main() -> None:
    ...
    normalize_generated_platform_pages(output)
