...
def check() -> list[str]:
    ...
    for built in built_pages:
        ...
        if built_text.count('class="nav-btn" type="button"') != 1 or built_text.count('aria-label="Open primary navigation"') != 1:
            errors.append(f"{rel}: mobile navigation state control missing or duplicated")
    ...
    for rel in generated_offers:
        ...
        if "offers.css" in text:
            errors.append(f"{rel}: detached offer stylesheet must not be loaded")
        if text.count("style.css?v=") != 2:
            errors.append(f"{rel}: offer page must preload and load the shared visual system stylesheet")
